...
contract MyToken is ERC721, ERC721URIStorage, AccessControl {
    ...
    uint256 private init_tokenId = 0;  
    ...
    constructor(address defaultAdmin) ERC721("MyToken", "MTK") payable {
    ...
    function approveRequest(uint256 id, string memory uri) public onlyRole(MINTER_ROLE) {
        userRequests[id].status = true;
        uint256 tokenId = init_tokenId;
        init_tokenId++;

        safeMint(userRequests[id].requester, tokenId, uri);
        _transfer(msg.sender, userRequests[id].requester, tokenId);
